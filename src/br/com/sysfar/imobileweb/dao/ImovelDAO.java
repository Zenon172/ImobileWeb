...
public final class ImovelDAO implements CrudDAO<ImovelModel> {
	...
	public List<ImovelModel> pesquisar(final ProprietarioModel model) {
		...
		broker.setSQL("SELECT I.ID, I.FLAG_ATIVO, I.CODIGO, I.TIPO_IMOVEL_ID, (SELECT TI.DESCRICAO FROM TIPO_IMOVEL TI WHERE TI.ID = I.TIPO_IMOVEL_ID), I.CAPTADOR_ID, (SELECT C.NOME FROM USUARIO C WHERE C.ID = I.CAPTADOR_ID), I.BAIRRO_ID, (SELECT B.DESCRICAO FROM BAIRRO B WHERE B.ID = I.BAIRRO_ID), I.EDIFICIO_ID, (SELECT E.DESCRICAO FROM EDIFICIO E WHERE E.ID = I.EDIFICIO_ID), I.VALOR, I.ENDERECO, I.COMPLEMENTO, I.ANDAR, I.UNIDADE, I.PONTO_REFERENCIA, I.CEP, I.OBSERVACOES, I.POSICAO_SOL_ID, (SELECT PS.DESCRICAO FROM POSICAO_SOL PS WHERE PS.ID = I.POSICAO_SOL_ID), I.LATITUDE, I.LONGITUDE FROM IMOVEL I WHERE I.PROPRIETARIO_ID = ? ", model.getId());

		return broker.getCollectionBean(ImovelModel.class, "id", "flagAtivo", "codigo", "tipoImovelModel.id", "tipoImovelModel.descricao", "captadorModel.id", "captadorModel.nome", "bairroModel.id", "bairroModel.descricao", "edificioModel.id", "edificioModel.descricao", "valor", "endereco", "complemento", "andar", "unidade", "pontoReferencia", "cep", "observacoes", "posicaoSolModel.id", "posicaoSolModel.descricao", "latitude", "longitude");
	...
	public List<ImovelModel> pesquisarHome() {
		...
		broker.setSQL("SELECT I.ID, I.FLAG_ATIVO, I.CODIGO, I.TIPO_IMOVEL_ID, (SELECT TI.DESCRICAO FROM TIPO_IMOVEL TI WHERE TI.ID = I.TIPO_IMOVEL_ID), I.CAPTADOR_ID, (SELECT C.NOME FROM USUARIO C WHERE C.ID = I.CAPTADOR_ID), I.BAIRRO_ID, (SELECT B.DESCRICAO FROM BAIRRO B WHERE B.ID = I.BAIRRO_ID), (SELECT C.NOME FROM CIDADE C, BAIRRO B WHERE C.ID = B.CIDADE_ID AND B.ID = I.BAIRRO_ID), I.EDIFICIO_ID, (SELECT E.DESCRICAO FROM EDIFICIO E WHERE E.ID = I.EDIFICIO_ID), I.VALOR, I.VALOR_ALUGUEL, I.ENDERECO, I.COMPLEMENTO, I.ANDAR, I.UNIDADE, I.PONTO_REFERENCIA, I.CEP, I.OBSERVACOES, I.POSICAO_SOL_ID, (SELECT PS.DESCRICAO FROM POSICAO_SOL PS WHERE PS.ID = I.POSICAO_SOL_ID), (SELECT IF.ARQUIVO FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO ORDER BY IF.ORDEM LIMIT 1), (SELECT IF.FLAG_PORTRAIT FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO ORDER BY IF.ORDEM LIMIT 1), I.QUARTOS, I.SUITES, I.QTD_VAGAS_ESTACIONAMENTO, I.AREA_PRIVATIVA, I.LATITUDE, I.LONGITUDE, I.TITULO_ANUNCIO, I.DESCRICAO_ANUNCIO FROM IMOVEL I WHERE I.FLAG_ATIVO AND EXISTS(SELECT 1 FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO) ORDER BY ID DESC");

		return broker.getCollectionBean(ImovelModel.class, "id", "flagAtivo", "codigo", "tipoImovelModel.id", "tipoImovelModel.descricao", "captadorModel.id", "captadorModel.nome", "bairroModel.id", "bairroModel.descricao", "bairroModel.cidadeModel.nome", "edificioModel.id", "edificioModel.descricao", "valor", "valorAluguel", "endereco", "complemento", "andar", "unidade", "pontoReferencia", "cep", "observacoes", "posicaoSolModel.id", "posicaoSolModel.descricao", "imagemFotoPrincipalModel.arquivo", "imagemFotoPrincipalModel.flagPortrait", "quartos", "suites", "qtdVagasEstacionamento", "areaPrivativa", "latitude", "longitude", "tituloAnuncio", "descricaoAnuncio");

	}

	@SuppressWarnings("unchecked")
	public List<ImovelModel> pesquisarImoveisRelacionados(ImovelModel imovelModel) {
		
		TSDataBaseBrokerIf broker = TSDataBaseBrokerFactory.getDataBaseBrokerIf();
		
		broker.setSQL("SELECT I.ID, I.FLAG_ATIVO, I.CODIGO, I.TIPO_IMOVEL_ID, (SELECT TI.DESCRICAO FROM TIPO_IMOVEL TI WHERE TI.ID = I.TIPO_IMOVEL_ID), I.CAPTADOR_ID, (SELECT C.NOME FROM USUARIO C WHERE C.ID = I.CAPTADOR_ID), I.BAIRRO_ID, (SELECT B.DESCRICAO FROM BAIRRO B WHERE B.ID = I.BAIRRO_ID), (SELECT C.NOME FROM CIDADE C, BAIRRO B WHERE C.ID = B.CIDADE_ID AND B.ID = I.BAIRRO_ID), I.EDIFICIO_ID, (SELECT E.DESCRICAO FROM EDIFICIO E WHERE E.ID = I.EDIFICIO_ID), I.VALOR, I.VALOR_ALUGUEL, I.ENDERECO, I.COMPLEMENTO, I.ANDAR, I.UNIDADE, I.PONTO_REFERENCIA, I.CEP, I.OBSERVACOES, I.POSICAO_SOL_ID, (SELECT PS.DESCRICAO FROM POSICAO_SOL PS WHERE PS.ID = I.POSICAO_SOL_ID), (SELECT IF.ARQUIVO FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO ORDER BY IF.ORDEM LIMIT 1), (SELECT IF.FLAG_PORTRAIT FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO ORDER BY IF.ORDEM LIMIT 1), I.QUARTOS, I.SUITES, I.QTD_VAGAS_ESTACIONAMENTO, I.AREA_PRIVATIVA, I.LATITUDE, I.LONGITUDE, I.TITULO_ANUNCIO, I.DESCRICAO_ANUNCIO FROM IMOVEL I WHERE I.FLAG_ATIVO AND I.VALOR BETWEEN (?::NUMERIC - (I.VALOR * 25/100)) AND (?::NUMERIC + (I.VALOR * 25/100)) ORDER BY RANDOM() DESC LIMIT 4", imovelModel.getValor(), imovelModel.getValor());
	...
	public List<ImovelModel> pesquisar(final ImovelModel model, String tipoOrdenacao) {
		...
		if ("menorCodigo".equals(tipoOrdenacao)) {

			query.append(" ORDER BY I.CODIGO ASC ");

		} else if ("maiorCodigo".equals(tipoOrdenacao)) {

			query.append(" ORDER BY I.CODIGO DESC ");

		} else if ("menorPreco".equals(tipoOrdenacao)) {

			query.append(" ORDER BY COALESCE(I.VALOR, I.VALOR_ALUGUEL) ASC ");

		} else if ("maiorPreco".equals(tipoOrdenacao)) {

			query.append(" ORDER BY COALESCE(I.VALOR, I.VALOR_ALUGUEL) DESC ");

		} else if ("menorTamanho".equals(tipoOrdenacao)) {

			query.append(" ORDER BY I.AREA_PRIVATIVA ASC ");

		} else if ("maiorTamanho".equals(tipoOrdenacao)) {

			query.append(" ORDER BY I.AREA_PRIVATIVA DESC ");

		} else if ("menorMetroQuadrado".equals(tipoOrdenacao)) {

			query.append(" ORDER BY COALESCE(I.VALOR, I.VALOR_ALUGUEL) * I.AREA_PRIVATIVA ASC ");

		} else if ("maiorMetroQuadrado".equals(tipoOrdenacao)) {

			query.append(" ORDER BY COALESCE(I.VALOR, I.VALOR_ALUGUEL) * I.AREA_PRIVATIVA DESC ");
		...
		broker.setSQL(query.toString(), model.getFlagAtivo(), Utilitario.getStringIlike(model.getCodigo(), true), model.getTipoImovelModel().getId(), model.getCaptadorModel().getId(), model.getValorMin(), model.getValorMax(), model.getValorCondominioMin(), model.getValorCondominioMax(), model.getQuartosMin(), model.getQuartosMax(), model.getProprietarioModel().getId(), model.getCondominioModel().getId(), model.getBairroModel().getId());
	...
	public List<ImovelModel> pesquisar(final ImovelModel model) {
		return this.pesquisar(model, null);
	}
	...
	public List<ImovelModel> pesquisar(String dadosProprietario) {
		...
		broker.setSQL("SELECT I.ID, I.FLAG_ATIVO, I.CODIGO, I.TIPO_IMOVEL_ID, (SELECT TI.DESCRICAO FROM TIPO_IMOVEL TI WHERE TI.ID = I.TIPO_IMOVEL_ID), I.CAPTADOR_ID, (SELECT C.NOME FROM USUARIO C WHERE C.ID = I.CAPTADOR_ID), I.BAIRRO_ID, (SELECT B.DESCRICAO FROM BAIRRO B WHERE B.ID = I.BAIRRO_ID), I.EDIFICIO_ID, (SELECT E.DESCRICAO FROM EDIFICIO E WHERE E.ID = I.EDIFICIO_ID), I.VALOR, I.LATITUDE, I.LONGITUDE FROM IMOVEL I, PROPRIETARIO P WHERE P.ID = I.PROPRIETARIO_ID AND SEM_ACENTOS(BUSCA_PROPRIETARIO(P.ID)) ILIKE SEM_ACENTOS(COALESCE(?, SEM_ACENTOS(BUSCA_PROPRIETARIO(P.ID)))) ", Utilitario.getStringIlike(dadosProprietario, true));

		return broker.getCollectionBean(ImovelModel.class, "id", "flagAtivo", "codigo", "tipoImovelModel.id", "tipoImovelModel.descricao", "captadorModel.id", "captadorModel.nome", "bairroModel.id", "bairroModel.descricao", "edificioModel.id", "edificioModel.descricao", "valor", "latitude", "longitude");
	}

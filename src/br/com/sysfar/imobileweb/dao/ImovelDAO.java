...
public final class ImovelDAO implements CrudDAO<ImovelModel> {
	...
	public List<ImovelModel> pesquisarHome() {
		...
		broker.setSQL("SELECT I.ID, I.FLAG_ATIVO, I.CODIGO, I.TIPO_IMOVEL_ID, (SELECT TI.DESCRICAO FROM TIPO_IMOVEL TI WHERE TI.ID = I.TIPO_IMOVEL_ID), I.CAPTADOR_ID, (SELECT C.NOME FROM USUARIO C WHERE C.ID = I.CAPTADOR_ID), I.BAIRRO_ID, (SELECT B.DESCRICAO FROM BAIRRO B WHERE B.ID = I.BAIRRO_ID), (SELECT C.NOME FROM CIDADE C, BAIRRO B WHERE C.ID = B.CIDADE_ID AND B.ID = I.BAIRRO_ID), I.EDIFICIO_ID, (SELECT E.DESCRICAO FROM EDIFICIO E WHERE E.ID = I.EDIFICIO_ID), I.VALOR, I.ENDERECO, I.COMPLEMENTO, I.ANDAR, I.UNIDADE, I.PONTO_REFERENCIA, I.CEP, I.OBSERVACOES, I.POSICAO_SOL_ID, (SELECT PS.DESCRICAO FROM POSICAO_SOL PS WHERE PS.ID = I.POSICAO_SOL_ID), (SELECT IF.ARQUIVO FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID ORDER BY IF.ORDEM LIMIT 1), (SELECT IF.FLAG_PORTRAIT FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID ORDER BY IF.ORDEM LIMIT 1), I.QUARTOS, I.SUITES, I.AREA_PRIVATIVA FROM IMOVEL I WHERE I.FLAG_ATIVO AND EXISTS(SELECT 1 FROM IMOVEL_FOTO IF WHERE IF.IMOVEL_ID = I.ID AND IF.FLAG_ATIVO) ORDER BY ID DESC");

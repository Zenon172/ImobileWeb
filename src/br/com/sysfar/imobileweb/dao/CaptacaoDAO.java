...
public final class CaptacaoDAO implements CrudDAO<CaptacaoModel> {
	...
	public CaptacaoModel inserir(final CaptacaoModel model) throws TSApplicationException {
		...
		broker.setSQL("INSERT INTO CAPTACAO(ID, TIPO_IMOVEL_ID, DESCRICAO, VALOR, BAIRRO_ID, ORIGEM_ID, CONTATO, DATA_ANUNCIO, LINK, DATA_CADASTRO, USUARIO_CADASTRO_ID, STATUS_CAPTACAO_ID, OBSERVACAO, RESPONSAVEL_ID, FLAG_OPORTUNIDADE, FLAG_SEM_LIQUIDEZ, TITULO) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", model.getId(), model.getTipoImovelModel().getId(), model.getDescricao(), model.getValor(), model.getBairroModel().getId(), model.getOrigemModel().getId(), model.getContato(), model.getDataAnuncio(), model.getLink(), new Timestamp(model.getDataCadastro().getTime()), model.getUsuarioCadastroModel().getId(), model.getStatusCaptacaoModel().getId(), model.getObservacao(), model.getResponsavelModel().getId(), model.getFlagOportunidade(), model.getFlagSemLiquidez(), model.getTitulo());
